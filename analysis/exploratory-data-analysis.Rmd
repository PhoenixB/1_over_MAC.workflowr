...
one_over_mac %>%
  select(pid, age, time, ce_mac, peak_alpha) %>%
  print(n = 20)

one_over_mac %>%
  summarise(across(c(pid, time, age, ce_mac, peak_alpha),
                   ~ sum(is.na(.x)))) %>%
...
one_over_mac_filtered %>%
  ...
  ggplot(aes(x = as.integer(pid), y = value, colour = lag)) +
  ...
  geom_point() +
  labs(x = "Patient", y = "Autocorrelation",
       title = "Autocorrelation by variable", colour = "Lag") +
...
one_over_mac_filtered %>%
  ...
  geom_point() +
  labs(x = "Patient", y = "Partial autocorrelation",
       title = "Partial autocorrelation by variable", colour = "Lag") +
  scale_colour_viridis_d() +
  theme_dark()
